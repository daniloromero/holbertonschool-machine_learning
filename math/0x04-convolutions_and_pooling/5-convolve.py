#!/usr/bin/env python3
"""Module that performs strided convolution on images with multiple kernels"""
import numpy as np


def convolve(images, kernels, padding='same', stride=(1, 1)):
    """performs a strided convolution on images with multiple kernels
    Args:
    ...
    kernel_w, kernel_h, c, nc = kernels.shape
    sh, sw = stride
    if padding == 'same':
    ...
    input_padded = np.pad(images, ((0, 0), (ph, ph), (pw, pw), (0, 0)),
                          'constant')

    output = np.zeros((m, h_pad, w_pad, nc))
    ...
    for k in range(nc):
        for i in range(h_pad):
            for j in range(w_pad):
                s_i = i * sh
                s_j = j * sw
                window = input_padded[img_size, s_i:kernel_w+s_i,
                                      s_j:kernel_h+s_j, :]
                kernel = kernels[:, :, :, k]
