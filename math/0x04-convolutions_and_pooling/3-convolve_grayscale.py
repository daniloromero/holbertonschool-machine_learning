#!/usr/bin/env python3
"""Module that performs strided convolution on grayscale images"""
import numpy as np


def convolve_grayscale(images, kernel, padding='same', stride=(1, 1)):
    """performs a strided convolution on grayscale images with custom paddind
    Args:
    ...
    sh, sw = stride
    if padding == 'same':
        ph = int((((h - 1) * sh + kernel_w - input_w) / 2) + 1)
        pw = int((((w - 1) * sw + kernel_h - input_h) / 2) + 1)
    elif padding == 'valid':
        ph = 0
        pw = 0
    else:
